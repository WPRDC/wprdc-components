...
//
// export const profilesDataVizConnection: MapPluginConnection<
//   DataVizID,
//   Downloaded<MiniMapViz, MiniMapOptions>
// > = {
//   getSources: (items, _, setSources) => {
//     if (!!items) {
//       const requests = items.map((item) => ProfilesAPI.requestDataViz(item));
...
//     const selectedItems =
//       selected === 'all' ? items.map((i) => i.id) : selected;
//
//     setLayers(
//       Array.from(selectedItems).reduce<LayerProps[]>(
//         (result, item) => [
//           ...result,
//           ...makeLayers(item as GeographyType, hoveredFilter, selectedFilter),
//         ],
//         []
//       )
//     );
